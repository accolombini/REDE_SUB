'''
    REDE_SUB.docx
        ...
        ||> Objetivo: consolidar Dados

            |> Combinar os dados coletados em campo (dados_campo.csv) com os dados da matriz de priorização gerada no Épico 3.
            |> Identificar discrepâncias entre os dados.

        ||> Gerar Ajustes:

            |> Aplicar ajustes ao modelo de criticidade com base nos dados de campo.
        
        ||> Relatório Final:

            |> Produzir um relatório consolidado (PDF) com os resultados da análise e ajustes realizados.
...
def calculate_criticidade_matriz(matriz_data):
    """
    Calcula a coluna Criticidade_Matriz com base nas colunas existentes.
    ...
    matriz_data["Criticidade_Matriz"] = (
        0.4 * matriz_data["Frequencia_Falhas"] +
        0.4 * matriz_data["Impacto_DEC_FEC"] +
        0.2 * matriz_data["Ponderacao_Total"]
    )
    return matriz_data


def consolidate_data(field_data_path, matriz_data_path, output_path):
    """
    ...
    if "Criticidade_Matriz" not in matriz_data.columns:
        ...
        matriz_data = calculate_criticidade_matriz(matriz_data)
...
def main():
    ...
    consolidated_data = consolidate_data(
        field_data_path, matriz_data_path, consolidated_data_path)
